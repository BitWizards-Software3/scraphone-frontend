...
class ProductRepository {
  ...
  Future<void> updateProduct(
      String productId, GetProductModel updatedProduct) async {
  ...
  Future<void> deleteProduct(String productId) async {
    final deleteUrl = Uri.parse('http://127.0.0.1:8000/products/$productId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Manejar errores aquí
        print(
            'Error al aliminar el producto. Código de estado: ${response.statusCode}');
        print(utf8.decode(response.bodyBytes));
      }
    } catch (error) {
      // Manejar errores de red aquí
      print('Error de red al eliminar el producto: $error');
    }
  }
  ...
  Future<List<GetProductModel>> fetchProducts() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<GetProductModel> products =
          productData.map((json) => GetProductModel.fromJson(json)).toList();
